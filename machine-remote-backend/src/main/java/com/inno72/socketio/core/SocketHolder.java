...
public class SocketHolder {
	...
	public static boolean send(String key, String msgType, Object data) {
	...
	static boolean send(SocketIOClient channel, String event, Object data) {
		if (channel != null) {
			channel.sendEvent(event, data);
			return true;
		} else {
			logger.info("连接不存在或以关闭, 不发送{}", event);
			return false;
		}
	}
